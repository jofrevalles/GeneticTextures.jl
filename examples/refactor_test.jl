...
GeneticTextures.primitives_with_arity = Dict(
    :sin => 1,
    :cos => 1,
    :tan => 1,
    :perlin_color => 2,
...
function evolve_system_step_threaded!(vals, dynamics::DynamicalSystem3, custom_exprs, width, height, t, dt, complex_func::Function)
...
function evolve_system_step_vectorized!(vals, dynamics::DynamicalSystem3, custom_exprs, width, height, t, dt, complex_func::Function)
...
function evolve_system_step_threadandvectorized!(vals, dynamics::DynamicalSystem3, custom_exprs, width, height, t, dt, complex_func::Function)
    # Prepare δvals to accumulate changes
    δvals = [Matrix{Color}(undef, height, width) for _ in 1:length(dynamics)]
...
function animate_system_2threaded(dynamics::DynamicalSystem3, width, height, T, dt; normalize_img = false, color_expr::Expr = :((vals...) -> RGB(sum(red.(vals))/length(vals), sum(green.(vals))/length(vals), sum(blue.(vals))/length(vals))), complex_expr::Expr = :((c) -> real(c)))
    ...
    for (i, t) in enumerate(range(0, T, step=dt))
        vals = evolve_system_step_threaded!(vals, dynamics, custom_exprs, width, height, t, dt, complex_func) # Evolve the system
...
# ds = DynamicalSystem3([A, B])
# width = height = 512
# animate_system_2threaded(ds, width, height, 50.0, 0.1; color_expr, complex_expr, normalize_img=true)
