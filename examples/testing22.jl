...
F_dA = :(ifs(abs(A) > 3, A^2 + Complex(0.3,-0.01) - A/$dt, A^3 - A/$dt))
# F_dA = :(A*x-1*A/$dt)

F_B0 = :(0) # C will be used as a counter
F_dB = :(ifs(abs(A) > 2, -1/$dt, 1/$dt)) # stop iterating when A escapes

F_C0 = :(Complex(4*x, 4*y))
F_dC = :(ifs(abs(C) > 2, 0, C^2 + Complex(0.4,0.02) - C/$dt))
F_dA = :(ifs(abs(A) > 2, 0, A^2 + C + Complex(0.3,-0.01) - A/$dt))
...
value_expr = :(begin
    ...
    (a, b,c) -> begin
        ...
        modified_k = continuous_potential(b.r, c.r)
    end
end)
