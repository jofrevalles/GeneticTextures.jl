...
function neighbor_max(expr, vars, width, height; Δx = 1, Δy = 1)
    ...
    max_val = expr(merge(vars, Dict([k => (isa(v, Matrix) ? v[idx_x, idx_y] : v) for (k, v) in vars])))
...
function neighbor_ave(func, vars, width, height; Δx = 1, Δy = 1)
    ...
    for dx in range_x, dy in range_y
        ...
        # temp_vars = merge(temp_vars, Dict(k => (isa(v, Matrix) ? v[idx_x + dx, idx_y + dy] : v) for (k, v) in temp_vars))
        # println("values in : $([(k, v) for (k, v) in temp_vars])")
        # println("typeof values: $([typeof(v) for (k, v) in temp_vars])")
        # println("values in : $([typeof(v) for (k, v) in merge(vars, Dict(k => (isa(v, Matrix) ? v[idx_x, idx_y] : v) for (k, v) in temp_vars))])")
